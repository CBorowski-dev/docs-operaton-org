...
This document guides you through the update from Operaton `7.14.x` to `7.15.0`. It covers these use cases:

1. For administrators and developers: [Database Updates](#database-updates)
1. For administrators and developers: [Full Distribution Update](#full-distribution)
1. For administrators: [Standalone Web Application](#standalone-web-application)
1. For developers: [Exception Handling in Task API](#exception-handling-in-task-api)
1. For administrators and developers: [Update of MySQL JDBC Driver in Operaton Docker Images](#update-of-mysql-jdbc-driver-in-camunda-docker-images)
...
1. For administrators and developers: [Changes to OperatonRun Start Script](#changes-to-camunda-platform-run-start-script)
...
3. We highly recommend to also check for any existing patch scripts for your database that are within the bounds of the new minor version you are updating to. Execute them in ascending order by version number. _Attention_: This step is only relevant when you are using an enterprise version of Operaton, e.g., `7.15.X` where `X > 0`. The procedure is the same as in step 1, only for the new minor version.
...
Custom Scripts created for Operaton or earlier can be included using the `requireJsConfig` property to the `app/{appName}/scripts/config.js`. You can include these custom scripts using the custom [requireJS configuration](https://requirejs.org/docs/api.html#config).
...
Plugins created for Operaton or earlier can be included for compatibility. To achieve this, simply prefix your Plugin ID with `legacy-`. The AngularJS module name for the entry module will be `{appName}.plugin.legacy-*`. There are 3 Steps you can follow to adjust your existing plugin to work with Operaton. We will show the changes with the [Cockpit Failed Jobs Plugin](https://github.com/camunda/camunda-bpm-examples/tree/7.13/cockpit/cockpit-failed-jobs-plugin), but the steps are the same for all webapp plugins.
  ...
  1. Prefix the Plugin ID with `legacy-`.
        ...
        @@ -24,7 +24,7 @@ import org.operaton.bpm.cockpit.plugin.spi.impl.AbstractCockpitPlugin;
...
# Changes to OperatonRun Start Script
With 7.15, we have introduced Swagger UI as an additional module of the OperatonRun distro. This brings changes to the
...
You can read more on the configuration of OperatonRun [here][run-documentation].
...
Starting with version 7.15.0, OperatonRuntime and its compatible Java External Task Client
