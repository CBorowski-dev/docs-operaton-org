...
This document guides you through the update from Operaton `7.9.x` to `7.10.0`. It covers these use cases:
...
3. We highly recommend to also check for any existing patch scripts for your database that are within the bounds of the new minor version you are updating to. Execute them in ascending order by version number. _Attention_: This step is only relevant when you are using an enterprise version of Operaton, e.g., `7.10.X` where `X > 0`. The procedure is the same as in step 1, only for the new minor version.
...
Starting from Operaton, only a single WildFly distribution, packaged with the latest version of the WildFly application server will be provided. For WildFly 10+, the update process remains the same.
...
If you are using Operaton Spring Boot Starter within you Spring Boot application, then you would need to:
...
please make sure to enable the `CsrfPreventionFilter` when migrating to 7.10 by adjusting the `web.xml` file of OperatonWebapps.
...
With Operaton a whitelist pattern of User, Group and Tenant IDs has been introduced. By default, on creating or
...
Operaton[supports scripting]({{< ref "/user-guide/process-engine/scripting.md" >}}) with JSR-223 compatible
...
Historic instances which (1) have been produced by a Operatonversion prior to 7.10 and (2) belong to a top-level instance
...
{{< note title="Heads Up!" class="info" >}}
Starting with Operaton there exist several Batch Operations to set a removal time to historic processes, decisions and batches.
This allows you to set a removal time via Operaton Cockpit as well as REST & Java API.
