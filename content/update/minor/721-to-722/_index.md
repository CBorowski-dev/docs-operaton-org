...
This document guides you through the update from Operaton `7.21.x` to `7.22.0` and covers the following use cases:

1. For administrators and developers: [Database updates](#database-updates)
1. For administrators and developers: [Full distribution update](#full-distribution)
1. For administrators and developers: [Operaton Spin](#camunda-spin)
1. For developers: [Operaton Commons](#camunda-commons)
1. For developers: [Operaton Template Engines FreeMarker](#camunda-template-engines-freemarker)
1. For developers: [Operaton Connect](#camunda-connect)
1. For developers: [Operaton Connect dependency removed from `camunda-engine`](#camunda-connect-dependency-removed-from-camunda-engine)
...
1. For administrators and developers: [Operaton Run and Swagger Update](#camunda-run-and-swagger-update)
...
With this release, we support JBoss EAP 8.0, it's Jakarta EE compliant platform. The artifacts are shipped with the latest pre-packaged [OperatonWildFly distribution]({{< ref "/installation/full/jboss/manual.md#setup" >}}).
...
* Replace Operaton class references:
  * `org.operaton.bpm.application.impl.EjbProcessApplication` → `org.operaton.bpm.application.impl.JakartaEjbProcessApplication`
  * `org.operaton.bpm.application.impl.ServletProcessApplicationDeployer` → `org.operaton.bpm.application.impl.JakartaServletProcessApplicationDeployer`
  * `org.operaton.bpm.application.impl.ServletProcessApplication` → `org.operaton.bpm.application.impl.JakartaServletProcessApplication`
  * `org.operaton.bpm.engine.impl.cfg.jta.JtaTransactionContext` → `org.operaton.bpm.engine.impl.cfg.jta.JakartaTransactionContext`
  * `org.operaton.bpm.engine.impl.cfg.jta.JtaTransactionContextFactory` → `org.operaton.bpm.engine.impl.cfg.jta.JakartaTransactionContextFactory`
  * `org.operaton.bpm.engine.impl.cfg.JtaProcessEngineConfiguration` → `org.operaton.bpm.engine.impl.cfg.JakartaTransactionProcessEngineConfiguration`
  * `org.operaton.bpm.engine.impl.interceptor.JtaTransactionInterceptor` → `org.operaton.bpm.engine.impl.interceptor.JakartaTransactionInterceptor`
* Replace Operaton Maven dependencies:
  * `org.operaton.bpm.javaee:camunda-ejb-client` → `org.operaton.bpm.javaee:camunda-ejb-client-jakarta`
  * `org.operaton.bpm:camunda-engine-cdi` → `org.operaton.bpm:camunda-engine-cdi-jakarta`
...
* Operaton WildFly Subsystem under `$JBOSS_HOME/modules/org/camunda/bpm/$APP_SERVER/camunda-wildfly-subsystem`
...
# Operaton Run and Swagger Update

Swagger UI was included in Operaton Run distros for a long time. Unfortunately, maintaining Swagger for Run was a lot of work. Other tools (like Postman or Swagger Editor) provide the same functionality outside of Operaton run. That is why we decided to discontinue Swagger in `camunda-run` to reduce maintenance efforts.

From now on, the `--swaggerui` argument is not available for Operaton Run start scripts, and the Swagger artifacts are not included in the Operaton Run distros anymore.
...
To continue running your Groovy scripts in the Operaton Engine, please read and act on the following release notes, especially the breaking changes:
...
To make the migration process easier, we have included the `groovy-dateutil` and `groovy-datetime` libraries in the Operaton Platform release. `groovy-dateutil` contains the extension methods for the `java.util.Date` class, while `groovy-datetime` allows using operator overloading, closures, and Groovy's dynamic methods on `java.time`.

## What if I can't migrate my scripts?

It is possible to keep using Operaton Platform with a lower version of Groovy if GROOVY-VERSION is the desired version.

### If using a full distribution of Operaton:
...
Sending telemetry data has been introduced in Operaton `7.14.0+` (and `7.13.7`, `7.12.12`, `7.11.19`)
 and removed in Operaton `7.22.0`. The public API is marked as deprecated and emptied out.
 ...
 The [diagnostic data][] is still being collected but not sent by any mean.
  You can decide to share it with Operaton if requested in tickets or use it for your diagnostic purposes.

In previous Operaton versions - `7.21.0+`, `7.20.8+`, `7.19.15+`, reporting telemetry data is disabled by default.
...
The Operaton Quarkus Extension has been updated to use Quarkus `3.15`. This  version brings its own features and changes.
...
The config properties now follow a similar scheme to the [Operaton Spring Boot Starter Configuration]({{< ref "/user-guide/spring-boot-integration/configuration.md#generic-properties" >}}).
...
The [Process Definition View][process-instance-modification] dialog of Operaton Cockpit has been updated to use the Historic API. With this change, the `start date` filter is now available within the workflow.
...
After migrating to the new version of Operaton, no saved filters will be available in the Modify Process Instance workflow. Users will need to re-enter and save these filters following the migration. Please notify your users before the migration.
{{< /note >}}
