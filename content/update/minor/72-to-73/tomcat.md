...
2. Update the optional Operatonlibraries
...
The pre-built Operaton distribution ships with Tomcat 7.0.62, whereas 7.2 comes with Tomcat 7.0.50. Operaton is supported on all Tomcat 6/7 versions such that a Tomcat update is not required when migrating from 7.2 to 7.3.
...
In addition to the core libraries, there may be optional artifacts in `$TOMCAT_HOME/lib/` for LDAP integration, Operaton Connect, and Operaton Spin. If you use any of these extensions, the following update steps apply:
...
2. Download the Operaton web application archive from our [Artifact Repository][artifact-repository]). Or switch to the private repository for the enterprise version (User and password from license required). Choose the correct version named `$PLATFORM_VERSION/camunda-webapp-tomcat-$PLATFORM_VERSION.war`.
3. Deploy the web application archive to your Tomcat instance.
