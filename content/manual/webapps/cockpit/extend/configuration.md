...
custom frontend module as described in [Cockpit Plugins - Structure of a Frontend Module]({{<ref "/manual/webapps/cockpit/extend/plugins.md#structure-of-a-frontend-module" >}}).
...
**Note:** Please make sure to change the CSRF cookie name also on [server-side]({{<ref "/manual/webapps/shared-options/csrf-prevention.md#cookie-name" >}}).
...
**Note:** This does only affect the Cockpit Operation Log. For the Admin Operation Log, check out the [Admin Configuration]({{<ref "/manual/webapps/admin/configuration.md#user-operation-log-annotation-length" >}}).
