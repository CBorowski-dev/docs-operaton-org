...
* Setting/getting variables in a [Delegate]({{< ref "/manual/user-guide/process-engine/delegation-code.md" >}})
...
{{< note title="Serializing Objects to XML and JSON" class="info" >}}
  The [Operaton Spin plugin]({{< ref "/manual/user-guide/data-formats/_index.md" >}}) provides serializers that are capable of serializing object values to XML and JSON. They can be used when it is desired that the serialized objects values can be interpreted by humans or when the serialized value should be meaningful without having the corresponding Java class. When using a pre-built Operaton distribution, Operaton Spin is already preconfigured and you can try these formats without further configuration.
{{< /note >}}
...
{{< note title="Java serialization format" class="warning" >}}
  ...
  with the help of the [`javaSerializationFormatEnabled`]({{< ref "/manual/reference/deployment-descriptors/tags/process-engine.md#javaSerializationFormatEnabled" >}}) configuration flag.
  However, please make sure to read the [Security Implication]({{< ref "/manual/user-guide/security.md#variable-values-from-untrusted-sources" >}}) first before enabling this.
{{< /note >}}

## JSON and XML Values

The Operaton Spin plugin provides an abstraction for JSON and XML documents that facilitate their processing and manipulation. This is often more convenient than storing such documents as plain `string` variables. See the documentation on Operaton SPIN for [storing JSON documents]({{< ref "/manual/user-guide/data-formats/json.md#native-json-variable-value" >}}) and [storing XML documents]({{< ref "/manual/user-guide/data-formats/xml.md#native-xml-variable-value" >}}) for details.
...
The default behavior is that the engine still tries to execute output mappings on canceled activities and fails with an exception if a variable is not found. By enabling the [skipOutputMappingOnCanceledActivities]({{< ref "/manual/reference/deployment-descriptors/tags/process-engine.md#skipOutputMappingOnCanceledActivities" >}}) engine configuration flag (i.e. setting it to `true`) the engine will not perform output mappings on any canceled activity.

[inputOutput]: {{< ref "/manual/reference/bpmn20/custom-extensions/extension-elements.md#camunda-inputoutput" >}}
[inputParameter]: {{< ref "/manual/reference/bpmn20/custom-extensions/extension-elements.md#inputparameter" >}}
[outputParameter]: {{< ref "/manual/reference/bpmn20/custom-extensions/extension-elements.md#outputparameter" >}}
[list]: {{< ref "/manual/reference/bpmn20/custom-extensions/extension-elements.md#camunda-list" >}}
[map]: {{< ref "/manual/reference/bpmn20/custom-extensions/extension-elements.md#camunda-map" >}}
[script-io]: {{< ref "/manual/user-guide/process-engine/scripting.md#use-scripts-as-inputoutput-parameters" >}}
