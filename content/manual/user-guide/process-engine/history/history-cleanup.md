...
History cleanup can be triggered manually or scheduled on a regular basis. Only [camunda-admins]({{< ref "/manual/user-guide/process-engine/authorization-service.md#the-camunda-admin-group">}}) have permissions to execute history cleanup manually.
...
The history clean up job does not delete historic [timer-start-event]({{< ref "/manual/reference/bpmn20/events/timer-events.md#timer-start-event" >}}) jobs. The reason being that the responsibility of timer-start-event job is to start a process instance, i.e. it does not belong to a process instance.
{{< /note >}}
...
* Changing the TTL of a definition only applies to history data that is created in the future. It does not dynamically update the removal time of already written history data. However, it is possible to [Set a Removal Time via Batch Operations]({{< ref "/manual/user-guide/process-engine/batch-operations.md#set-a-removal-time">}}).
...
* [Historic Instance Permissions]({{< ref "/manual/user-guide/process-engine/authorization-service.md#historic-instance-permissions" >}}) are not cleaned up.
* [History Cleanup Jobs]({{< ref "/manual/user-guide/process-engine/history/history-cleanup.md#historycleanupjobs-in-the-historic-job-log">}}) are not removed from the historic job log.

## Cleanup internals

History cleanup is implemented via jobs and performed by the [job executor]({{< ref "/manual/user-guide/process-engine/the-job-executor.md">}}). It therefore competes for execution resources with other jobs, e.g. triggering of BPMN timer events.
...
If cleanup fails, the job executor's [retry mechanism]({{< ref "/manual/user-guide/process-engine/the-job-executor.md#failed-jobs">}}) applies. Once the cleanup job has run out of retries, it is not executed again until one of the following actions is performed:
...
The history time to live is mandatory, any deployment or re-deployment of any model resource (BPMN, DMN, CMMN) that contains a historyTimeToLive of null will be prevented. Unless explicitly disabled via [process engine configuration]({{< ref "/manual/reference/deployment-descriptors/tags/process-engine.md#enforceHistoryTimeToLive">}}). To define a default TTL for process definitions and decision definitions if no other value is defined check [historyTimeToLive configuration]({{< ref "/manual/reference/deployment-descriptors/tags/process-engine.md#historyTimeToLive">}}).
...
Use the ["historyTimeToLive" extension attribute]({{< ref "/manual/reference/bpmn20/custom-extensions/extension-attributes.md#historytimetolive">}}) of the process definition to define the TTL for all its instances:
...
use the ["historyTimeToLive" attribute]({{< ref "/manual/reference/deployment-descriptors/tags/process-engine.md#historytimetolive">}})
...
The `batchOperationsForHistoryCleanup` property can be configured in Spring based application or via custom [Process Engine Plugins]({{< ref "/manual/user-guide/process-engine/process-engine-plugins.md">}}). It defines history time to live for each specific historic batch operation.
...
and cleanup must be configured explicitly. Please note that this only works for the [removal-time based history cleanup strategy]({{< ref "/manual/user-guide/process-engine/history/history-cleanup.md#removal-time-strategy">}}).
...
The process engine reports [runtime metrics]({{< ref "/manual/user-guide/process-engine/metrics.md">}}) to the database that can help draw conclusions about usage, load, and performance of the BPM platform.
...
[configuration-options]: {{< ref "/manual/reference/deployment-descriptors/tags/process-engine.md#history-cleanup-configuration-parameters">}}