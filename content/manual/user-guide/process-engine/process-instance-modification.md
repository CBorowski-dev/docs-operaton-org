...
Starting before an activity  via `startBeforeActivity` means that execution is started before entering the activity. The instruction respects an `asyncBefore` flag, meaning that a job will be created if the activity is `asyncBefore`. In general, this instruction executes the process model beginning with the specified activity until a wait state is reached. See the documentation on [Transactions in Processes]({{< ref "/manual/user-guide/process-engine/transactions-in-processes.md" >}}) for details on wait states.
...
See the [variables section of this guide]({{< ref "/manual/user-guide/process-engine/variables.md" >}}) for details on variables and scopes in general.
...
It is possible to execute modification of single process instance asynchronous. The [modification]({{< ref "/manual/user-guide/process-engine/process-instance-modification.md#modification-instruction-types" >}}) instructions are the same as the synchronous modification and the syntax of fluent builder is the following:
...
Batch modificationBatch = runtimeService.createProcessInstanceModification(processInstanceId)
        ...
        .executeAsync();
```
This would create a modification [batch]({{< ref "/manual/user-guide/process-engine/batch.md" >}}) which will be executed asynchronously.
...
section of the [user guide]({{< ref "/manual/user-guide/process-engine/process-instance-migration.md#executing-a-migration-plan" >}}).
...
runtimeService.createProcessInstanceModification(processInstanceId)
  ...
  .execute();
```
It will be visible in [User Operation Log]({{< ref "/manual/user-guide/process-engine/history/user-operation-log.md#annotation-of-user-operation-logs" >}}) for the performed modification.
