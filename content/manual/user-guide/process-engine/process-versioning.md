...
If you are using [multi-tenancy with tenant identifiers]({{< ref "/manual/user-guide/process-engine/multi-tenancy.md#single-process-engine-with-tenant-identifiers" >}}) then each tenant has its own process definitions which have versions independent from other tenants. See the [multi-tenancy section]({{< ref "/manual/user-guide/process-engine/multi-tenancy.md#versioning-of-tenant-specific-definitions" >}}) for details.
{{< /note >}}
...
When you use [BPMN CallActivities]({{< ref "/manual/reference/bpmn20/subprocesses/call-activity.md" >}}) you can configure which version is used:
...
[camunda:versionTag]({{< ref "/manual/reference/bpmn20/custom-extensions/extension-attributes.md#versiontag" >}})
...
[Process instance migration]({{< ref "/manual/user-guide/process-engine/process-instance-migration.md" >}}) can be used
