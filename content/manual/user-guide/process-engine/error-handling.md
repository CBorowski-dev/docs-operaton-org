...
The standard handling strategy is that exceptions are thrown to the client, meaning that the current transaction is rolled back. This means that the process state is rolled back to the last wait state. This behavior is described in detail in the [Transactions in Processes]({{< ref "/manual/user-guide/process-engine/transactions-in-processes.md" >}}) section of the [User Guide]({{< ref "/manual/user-guide/_index.md" >}}). Error handling is delegated to the client by the engine.
...
If you don't want the exception being shown to the user, one option is to make service calls, which might cause an error, async (as described in [Transactions in Processes]({{< ref "/manual/user-guide/process-engine/transactions-in-processes.md" >}})). In that case the exception is stored in the process engine database and the [Job]({{< ref "/manual/user-guide/process-engine/the-job-executor.md" >}}) in the background is marked as failed (to be more precise, the exception is stored and some retry counter is decremented).
...
See the [Error Events]({{< ref "/manual/reference/bpmn20/events/error-events.md" >}}) section of the [BPMN 2.0 Implementation Reference]({{< ref "/manual/reference/bpmn20/_index.md" >}}) and the [Throwing Errors from Delegation Code]({{< ref "/manual/user-guide/process-engine/delegation-code.md#throw-bpmn-errors-from-delegation-code" >}}) section of the [User Guide]({{< ref "/manual/user-guide/_index.md" >}}) for more information.

## BPMN 2.0 Compensation and Business Transactions

BPMN 2.0 transactions and compensations allow you to model business transaction boundaries (however, not in a technical ACID manner) and make sure already executed actions are compensated during a rollback. Compensation means to make the effect of the action invisible, e.g. book in goods if you have previously booked out the goods. See the [BPMN Compensation event]({{< ref "/manual/reference/bpmn20/events/cancel-and-compensation-events.md" >}}) and the [BPMN Transaction Subprocess]({{< ref "/manual/reference/bpmn20/subprocesses/transaction-subprocess.md" >}}) sections of the [BPMN 2.0 Implementation Reference]({{< ref "/manual/reference/bpmn20/_index.md" >}}) for details.
...
If you use Jobs (`async`), you can leverage Cockpit as monitoring tool to handle failed jobs, in this case no end user sees the exception. Then you normally see failures in cockpit when the retries are depleted (see the [Failed Jobs]({{< ref "/manual/user-guide/process-engine/the-job-executor.md#failed-jobs" >}}) section of the [Web Applications]({{< ref "/manual/webapps/cockpit/_index.md" >}}) for more information).

See the [Failed Jobs in Cockpit]({{< ref "/manual/webapps/cockpit/bpmn/failed-jobs.md" >}}) section of the [Web Applications]({{< ref "/manual/webapps/cockpit/_index.md" >}}) for more details.
...
You can access error codes via Java as well as [REST API]({{< ref "/manual/reference/rest/overview/_index.md#exception-codes" >}}).
...
Learn more on how to assign a custom error code to an exception in the documentation about [Delegation Code]({{< ref "/manual/user-guide/process-engine/delegation-code.md#exception-codes" >}}).

## Configuration

You can configure the exception error codes feature in your [process engine configuration]({{< ref "/manual/reference/deployment-descriptors/tags/process-engine.md#exception-codes" >}}):
...
With the help of a [`ProcessEnginePlugin`]({{< ref "/manual/user-guide/process-engine/process-engine-plugins.md" >}}) you can register a custom `{{< javadocref page="org/camunda/bpm/engine/impl/errorcode/ExceptionCodeProvider.html" text="ExceptionCodeProvider" >}}`:
