...
To execute an operation [asynchronously]({{< ref "/manual/user-guide/process-engine/batch.md">}}), only a **Create** permission on the Batch Resource is required. However, when executing the same operation synchronously, the specific permissions (e.g. **Delete** on **Process Instance Resource**) are checked.
...
The Operaton Admin Webapplication provides an out of the box [UI for configuring Authorizations]({{< ref "/manual/webapps/admin/authorization-management.md" >}}).
...
[hist-inst-perm-config-flag]: {{< ref "/manual/reference/deployment-descriptors/tags/process-engine.md#enable-historic-instance-permissions" >}}
[Removal-Time-based History Cleanup Strategy]: {{< ref "/manual/user-guide/process-engine/history/history-cleanup.md#removal-time-based-strategy" >}}
