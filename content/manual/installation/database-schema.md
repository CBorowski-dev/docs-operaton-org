...
menu:
  main:
    ...
    pre: "Install and update the Operaton database schema used by the Process Engine."

---

This document will guide you through the installation and update of the Operaton[database schema]({{< ref "/manual/user-guide/process-engine/database/database-schema.md" >}}) used by the process engine.
Regardless of the [architecture of your application setup]({{< ref "/manual/introduction/architecture.md" >}}), the process engine always requires this database schema.
...
Consult the installation guide related to your setup to configure the database for a [remote engine]({{< ref "/manual/installation/camunda-bpm-run.md" >}}), [shared engine]({{< ref "/manual/installation/full/_index.md" >}}), or [embedded engine]({{< ref "/manual/installation/spring-boot.md" >}}) accordingly.
...
Operatonsupports a variety of databases that are documented in the [supported environments]({{< ref "/manual/introduction/supported-environments.md#databases" >}}).

Operatonsupports the following ways of installing the database schema:

* Use the database migration tool [Liquibase](https://www.liquibase.org/) with the provided changelog for a semi-automatic installation and update. Liquibase keeps track of database schema changes. This allows you to focus on *when* changes should be applied rather than also on *which* changes are relevant. Starting with version `7.16.0`, Operatoncomes with a curated *changelog file* that Liquibase can consume.
...
READ COMMITED is the required isolation level for database systems to run Operatonwith.
You may have to change the default setting on your database when installing Operaton.
For more information see the documentation on [isolation levels]({{< ref "/manual/user-guide/process-engine/database/database-configuration.md#isolation-level-configuration" >}}).
{{< /note >}}

# Installation

You can either install your database schema using Liquibase or using the provided SQL scripts manually. You can switch between those mechanisms when updating your Operatonversion at a later stage if desired.
...
Operatoncomes with a maintained changelog file that Liquibase can consume.
...
Consult the [manual installation guide]({{< ref "/manual/installation/full/_index.md" >}}) of your distribution for further information on how to achieve that.
...
Updating to a newer Operatonminor version also requires a database schema update. You can reuse all the options available for a [schema installation](#installation) here as well.
...
1. We highly recommend updating to the latest patch version that is within the bounds of the new minor version you are updating to (`$Y`).
    ...
    _Attention_: This step is only relevant when you are using an enterprise version of Operaton, e.g., `7.16.X` where `X > 0`.
...
[pre-packaged distribution]: {{< ref "/manual/introduction/downloading-operaton.md#full-distribution" >}}
