...
* Install the database schema to create all required tables and default indices using our [database schema installation guide]({{< ref "/manual/installation/database-schema.md" >}}).
...
Note that you can configure multiple datasources used by different process engine instances. See the [User Guide]({{< ref "/manual/user-guide/_index.md" >}}) for details.
...
The names of the [JNDI Bindings for Operaton Services]({{< ref "/manual/user-guide/runtime-container-integration/jndi-bindings-for-bpmn-platform-services.md" >}}) consist of the name of the enterprise application deployment.
...
3. (optional) [Configure location of the bpm-platform.xml file]({{< ref "/manual/reference/deployment-descriptors/descriptors/bpm-platform-xml.md#configure-location-of-the-bpm-platform-xml-file" >}}).
