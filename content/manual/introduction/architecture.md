...
Operaton can be used both as a standalone process engine server or embedded inside custom Java applications. The embeddability requirement is at the heart of many architectural decisions within Operaton. For instance, we work hard to make the process engine component a lightweight component with as little dependencies on [third-party libraries]({{< ref "/manual/introduction/third-party-libraries/_index.md" >}}) as possible. Furthermore, the embeddability motivates programming model choices such as the capabilities of the process engine to participate in Spring Managed or JTA [transactions and the threading model]({{< ref "/manual/user-guide/process-engine/transactions-in-processes.md" >}}).
...
* [Job Executor]({{< ref "/manual/user-guide/process-engine/the-job-executor.md" >}}): The Job Executor is responsible for processing asynchronous background work such as Timers or asynchronous continuations in a process.
...
See the section on [third-party libraries]({{< ref "/manual/introduction/third-party-libraries/_index.md" >}}).
...
The process engine [job executor]({{< ref "/manual/user-guide/process-engine/the-job-executor.md" >}}) is also clustered and runs on each node. This way, there is no single point of failure as far as the process engine is concerned. The job executor can run in both [homogeneous and heterogeneous clusters]({{< ref "/manual/user-guide/process-engine/the-job-executor.md#cluster-setups" >}}).

{{< note title="Time zones" class="info" >}}
The are some limitations on [time zone usage in a cluster]({{< ref "/manual/user-guide/process-engine/time-zones.md#cluster-setup" >}}).
{{< /note >}}
...
More details can be found in the [multi-tenancy section]({{< ref "/manual/user-guide/process-engine/multi-tenancy.md" >}}).
...
* [camunda-bpmn.js](https://github.com/camunda/camunda-bpmn.js): Operaton BPMN 2.0 JavaScript libraries
